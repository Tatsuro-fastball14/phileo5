...
export default {
  ...
  methods: {
    fetchCurrentUser() {
      axios
        .get('/api/users/current') // Rails側でエンドポイントを設定する
        .then((response) => {
    ...
    handleSubmit() {
      // 検索処理のロジックをここに記述
      window.location.href = '/cooks_search' // または適切な検索ページへの遷移
    },
    registerDB() {
      // DB登録ページへの遷移
    ...
    deleteAccount() {
      if (confirm('本当にアカウントを削除しますか？')) {
        axios.delete(`/api/users/${this.currentUser.id}`).then(() => {
          alert('アカウントが削除されました')
    ...
    cancelSubscription() {
      if (confirm('本当に退会致しますか？')) {
        axios.delete(`/api/users/${this.currentUser.id}/subscription`).then(() => {
          alert('サブスクを退会しました')
        })
      }
    }
